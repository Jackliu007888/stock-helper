...
const url_get_stock_trade = 'http://vip.stock.finance.sina.com.cn/quotes_service/view/'
export function getStockTrade(code, num) {
  var num = num ? num : 20000
  return axios.get(`${url_get_stock_trade}CN_TransListV2.php?num=${num}&symbol=${code}&rn=${(Math.random(4).toString()).slice(2,10)}`).then(res => {
    return Promise.resolve(res.data)
  })
}